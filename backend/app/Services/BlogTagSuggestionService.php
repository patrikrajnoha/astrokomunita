...
class BlogTagSuggestionService
{
    ...
    public function __construct(
        ...
        private readonly BlogTagPromptBuilder $promptBuilder,
    ) {
    }
    ...
    public function suggestForPost(BlogPost $blogPost, string $mode = self::MODE_EXISTING_ONLY): array
    {
        ...
        try {
            $response = $this->ollamaClient->generate(
                prompt: $prompt,
                system: $this->promptBuilder->systemPromptForExistingTags(),
                options: [
    ...
    private function suggestAllowingNewTags(
        ...
        $prompt = $existingTags === []
            ? $this->promptBuilder->buildOpenPrompt(
                title: $title,
                content: $content
            )
            : $this->promptBuilder->buildFlexiblePrompt(
                title: $title,
                content: $content,
                existingTagNames: array_values(array_map(static fn (array $row): string => $row['name'], $existingTags)),
        ...
        $systemPrompt = $existingTags === []
            ? $this->promptBuilder->systemPromptForOpenTags()
            : $this->promptBuilder->systemPromptForFlexibleTags();
