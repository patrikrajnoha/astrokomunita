...
use App\Services\Crawlers\NasaWatchTheSkiesCrawlerService;
use Carbon\CarbonImmutable;
use Illuminate\Console\Command;

class CrawlNasaWatchTheSkiesEventsCommand extends Command
{
    protected $signature = 'events:crawl-nasa-wts
                            {--year= : Target year for moon phases}
                            {--from= : Local date YYYY-MM-DD}
                            {--to= : Local date YYYY-MM-DD}
                            {--dry-run : Parse and dedupe without inserts}';

    protected $description = 'Crawl NASA Watch the Skies (USNO moon phases) into event_candidates.';

    public function __construct(
        private readonly NasaWatchTheSkiesCrawlerService $crawler,
    ...
    public function handle(): int
    {
        ...
        $timezone = (string) config('events.source_timezones.nasa_watch_the_skies', 'UTC');
