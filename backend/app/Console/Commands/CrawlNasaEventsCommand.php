...
use App\Services\Crawlers\NasaCrawlerService;
use Carbon\CarbonImmutable;
use Illuminate\Console\Command;

class CrawlNasaEventsCommand extends Command
{
    protected $signature = 'events:crawl-nasa
                            {--year= : Target year for eclipse events}
                            {--from= : Local date YYYY-MM-DD}
                            {--to= : Local date YYYY-MM-DD}
                            {--dry-run : Parse and dedupe without inserts}';

    protected $description = 'Crawl NASA/USNO local solar eclipse visibility into event_candidates.';

    public function __construct(
        private readonly NasaCrawlerService $crawler,
    ...
    public function handle(): int
    {
        ...
        $timezone = (string) config('events.source_timezones.nasa', 'UTC');
