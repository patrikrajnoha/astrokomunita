...
- Optional grammar quality pass is provider-based (`GrammarCheckerInterface`):
  - implementations: `languagetool` (rule-based) or `ollama` (local LLM), both fail-open
- Event descriptions support 2 modes:
  - `template` (default): deterministic Slovak templates, stable and audit-friendly
  - `ollama`: local LLM generation via Ollama
  - command: `php artisan events:generate-descriptions --mode=template`
- Async jobs:
...
### Run Ollama (local open-source AI)
```powershell
ollama serve
ollama pull mistral
```
...
TRANSLATION_CACHE_KEY_VERSION=v7
...
TRANSLATION_GRAMMAR_OLLAMA_LANGUAGE=sk
TRANSLATION_GRAMMAR_OLLAMA_MODEL=mistral
TRANSLATION_GRAMMAR_OLLAMA_TEMPERATURE=0.0
TRANSLATION_GRAMMAR_OLLAMA_NUM_PREDICT=256
TRANSLATION_GRAMMAR_OLLAMA_TIMEOUT=20
...
OLLAMA_RETRY=1
OLLAMA_RETRY_SLEEP_MS=250
OLLAMA_VERIFY_SSL=true
OLLAMA_INTERNAL_TOKEN=
OLLAMA_TEMPERATURE=0.1
OLLAMA_NUM_PREDICT=256

EVENTS_DISPLAY_TIMEZONE=Europe/Bratislava
EVENTS_AI_DESCRIPTION_MODE=template
EVENTS_AI_MODEL=mistral
EVENTS_AI_TEMPERATURE=0.2
EVENTS_AI_NUM_PREDICT=420
EVENTS_AI_TIMEOUT=45
```
...
Generate/refresh event descriptions:

```powershell
php artisan events:generate-descriptions --dry-run
php artisan events:generate-descriptions --force
php artisan events:generate-descriptions --ids=42,43
php artisan events:generate-descriptions --mode=template --force
php artisan events:generate-descriptions --mode=ollama --limit=20
```
