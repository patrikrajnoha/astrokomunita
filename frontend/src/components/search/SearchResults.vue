      ...
      <section>
        ...
        <div v-if="recentSearches.length" class="flex flex-wrap gap-2">
          <button
            ...
            :key="item"
            type="button"
            class="rounded-full border border-[var(--divider-color)] bg-[color:rgb(var(--color-bg-rgb)/0.86)] px-3 py-1.5 text-xs text-[var(--color-surface)] transition hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]"
