<template>
  <div class="dashboard">
    <header class="dashboardHeader">
      ...
      <p class="dashboardSubtitle">Admin prehľad a metriky</p>
    </header>
...
export default {
  ...
  setup() {
    ...
    // Use useAdminTable for dashboard data fetching
    const {
      loading,
      error,
      data,
      ...
      async (params) => {
        const response = await http.get('/admin/dashboard', {
          params: { ...params, range: selectedRange.value }
        })
        return response
      },
      {
        autoFetch: false // Disable auto-fetch since we have custom range logic
      }
    )
    ...
    // Watch for range changes and refetch data
    watch(selectedRange, () => {
      fetchDashboardData()
    })
