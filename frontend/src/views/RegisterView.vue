...
const usernameIsAutoGenerated = ref(true)
const usernameAlternatives = ref([])
    ...
    usernameCheckTimer = setTimeout(async () => {
      ...
      try {
        ...
        if (data?.reason === 'taken') {
          usernameAlternatives.value = generateAlternatives(name.value)
        }
      } catch {
        if (requestId !== usernameCheckRequestId) return
...
watch(
  () => name.value,
  (nextName) => {
    if (!usernameIsAutoGenerated.value) return
    username.value = generateUsernameFromName(nextName)
  }
)
...
function generateUsernameFromName(nameValue) {
  const slug = slugifyName(nameValue)
  if (!slug) return ''
  const candidate = `astro_${slug}`.slice(0, 20).replace(/_+$/, '')
  return candidate.length >= 3 ? candidate : ''
}

function generateAlternatives(nameValue) {
  const firstWord = slugifyName((nameValue || '').split(/\s+/)[0])
  if (!firstWord) return []
  const suffix = Math.floor(Math.random() * 90) + 10
  const candidates = [
    `astro_${firstWord}_${suffix}`.slice(0, 20).replace(/_+$/, ''),
    `${firstWord}_nova`.slice(0, 20).replace(/_+$/, ''),
    `sky_${firstWord}`.slice(0, 20).replace(/_+$/, ''),
  ]
  return candidates.filter(
    (a) => a.length >= 3 && a.length <= 20 && /^[a-z][a-z0-9_]*$/.test(a) && !a.includes('__'),
  )
}

function onUsernameInput() {
  usernameIsAutoGenerated.value = false
  usernameAlternatives.value = []
}
